...
import 'package:hamy_yamy/features/restaurant/domain/models/cuisine_model.dart';
...
class RestaurantRepository implements RestaurantRepositoryInterface {
  ...
  Future<ProductModel?> getProductList(
      String offset, String type, String stockType, int? categoryId) async {
    ProductModel? productModel;
    Response response = await apiClient.getData(
        '${AppConstants.productListUri}?offset=$offset&limit=10&type=$type&category_id=$categoryId&stock=$stockType');
    if (response.statusCode == 200) {
  ...
  Future<bool> updateRestaurant(
    ...
    fields.addAll(<String, String>{
      ...
      // 'extra_packaging_status': restaurant.extraPackagingStatus.toString(),
      // 'extra_packaging_amount': restaurant.extraPackagingAmount.toString(),
      // 'is_extra_packaging_active':
      //     restaurant.isExtraPackagingActive! ? '1' : '0',
      // 'is_dine_in_active': restaurant.isDineInActive! ? '1' : '0',
      // 'schedule_advance_dine_in_booking_duration':
      //     restaurant.scheduleAdvanceDineInBookingDuration.toString(),
      // 'schedule_advance_dine_in_booking_duration_time_format':
      //     restaurant.scheduleAdvanceDineInBookingDurationTimeFormat ?? '',
      // 'customer_date_order_sratus':
      //     restaurant.customDateOrderStatus! ? '1' : '0',
      // 'customer_order_date': restaurant.customOrderDate.toString(),
      // 'free_delivery_distance_status':
      //     restaurant.freeDeliveryDistanceStatus! ? '1' : '0',
      // 'free_delivery_distance': restaurant.freeDeliveryDistance.toString(),
      // 'tags': tags,
    });
    ...
    Response response = await apiClient.postMultipartData(
        AppConstants.restaurantUpdateUri(restaurant.id),
        fields,
  ...
  Future<bool> addProduct(
      ...
      String nutrition,
      String allergicIngredients) async {

    Map<String, String> fields = {};
    if (kDebugMode) {
      print(
          '-========>R Ids>>>> :$deletedVariationIds // $deletedVariationOptionIds');
    }
    String deleteVariationIds = '';
    for (int index = 0; index < deletedVariationIds.length; index++) {
      deleteVariationIds =
          '$deleteVariationIds${index == 0 ? deletedVariationIds[index] : ',${deletedVariationIds[index]}'}';
    }
    String deleteVariationOptionIds = '';
    for (int index = 0; index < deletedVariationOptionIds.length; index++) {
      deleteVariationOptionIds =
          '$deleteVariationOptionIds${index == 0 ? deletedVariationOptionIds[index] : ',${deletedVariationOptionIds[index]}'}';
    }
    ...
    if (kDebugMode) {
      print(
          '============eee====> $deleteVariationIds // $deleteVariationOptionIds');
    }
    fields.addAll(<String, String>{
      ...
      'item_stock':
          product.itemStock != null ? product.itemStock.toString() : '0',
      'stock_type': product.stockType.toString(),
  ...
  Future<List<ReviewModel>?> getRestaurantReviewList(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.restaurantReviewUri}?restaurant_id=$restaurantID&search=$searchText');
    if (response.statusCode == 200) {
  ...
  Future<List<ReviewModel>?> getProductReviewList(int? productID) async {
    ...
    Response response =
        await apiClient.getData('${AppConstants.productReviewUri}/$productID');
    if (response.statusCode == 200) {
      productReviewList = [];
      response.body.forEach(
  ...
  Future<Product?> get(int id) async {
    Product? product;
    Response response =
        await apiClient.getData('${AppConstants.productDetailsUri}/$id');
    if (response.statusCode == 200) {
  ...
  Future<List<String?>?> getCharacteristicSuggestionList() async {
    ...
    Response response =
        await apiClient.getData(AppConstants.getCharacteristicSuggestion);
    if (response.statusCode == 200) {
      characteristicSuggestionList = [];
      response.body.forEach((characteristic) =>
          characteristicSuggestionList?.add(characteristic));
    }
  ...
  Future<bool> updateProductStock(String foodId, String itemStock,
    ...
    for (var variation in product.variations!) {
      for (var value in product
          .variations![product.variations!.indexOf(variation)]
          .variationValues!) {
        int optionId = int.parse(value.optionId.toString());
        String currentStock =
            variationStock[product.variations!.indexOf(variation)][product
                .variations![product.variations!.indexOf(variation)]
                .variationValues!
                .indexOf(value)];
    ...
    Response response =
        await apiClient.postData(AppConstants.productUpdateStock, fields);
    return (response.statusCode == 200);
  }
